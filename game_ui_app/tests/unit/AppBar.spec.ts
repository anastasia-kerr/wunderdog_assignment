import { RoundResult } from '@/api/types'
import Vue from 'vue'
import AppBar from '@/components/AppBar.vue'
import { RootState } from '@/store/types'
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils'
import Vuetify from 'vuetify'
import Vuex, { Store } from 'vuex'

const localVue = createLocalVue()
localVue.use(Vuex)

describe('AppBar', () => {
  let store: Store<RootState>
  let vuetify: Vuetify
  let wrapper:Wrapper<Vue>
  const player1 = 'player1'
  const player2 = 'player2'
  const roundNumber = 1
  const state = {
    gameId: '1',
    error: '',
    roundNumber: roundNumber,
    currentPlayers: [player1, player2],
    gameWinner: '',
    nickname: player1,
    playerIdentifier: player1 + 'playerIdentifier',
    roundResults: {} as RoundResult
  }
  beforeEach(() => {
    vuetify = new Vuetify()

    store = new Vuex.Store({
      state: { ...state }
    })

    wrapper = shallowMount(AppBar, { store, localVue, vuetify })
  })
  it('Must Display current users if there are any', () => {
    expect(wrapper.text()).toContain(player1)
    expect(wrapper.text()).toContain(player2)
  })
  it('Must Display current round number', () => {
    expect(wrapper.text()).toContain(roundNumber.toString())
  })

  it('Must not display player names if no current players available', () => {
    store = new Vuex.Store({
      state: {
        ...state,
        currentPlayers: [player1]
      }
    })
    wrapper = shallowMount(AppBar, { store, localVue, vuetify })

    expect(wrapper.text()).not.toContain(player1)
  })
})
